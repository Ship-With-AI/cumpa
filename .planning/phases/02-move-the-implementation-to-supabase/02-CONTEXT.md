...
- `src/server/support-client.ts`, `src/server/capabilities.ts`, and the app factories — Reuse the narrow hosted-client/capability boundary while replacing email-recovery contracts with hosted OAuth initiation and status polling; when `CUMPA_SUPPORT_SERVICE_URL` is absent, omit the support capability entirely rather than leaving unavailable no-op actions.
...
- Treat missing hosted configuration as an intentional development mode: no startup prompt, polling, support routes, or enabled Support/Restore controls.
...
- Local development must exercise the complete review product without Supabase; hosted support exists only when the production capability URL is explicitly configured.
