...
Test with `tests/unit/review-export.test.ts`, `tests/unit/review-markdown.test.ts`, and `tests/api/export*.test.ts`: parse final export schemas, compare deterministic output, assert patch identity/digest survives draft → export and cannot be spoofed or omitted.
