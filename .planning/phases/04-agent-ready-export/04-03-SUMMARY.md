...
- `633369b` gates the real compiler assertion to an actual `darwin-arm64` host while retaining portable injected stale-addon removal coverage. `b36fe78` moves the required build into serialized `npm run test:package-export-safety`, so concurrent package files never mutate shared `dist`; the standalone command rebuilt then passed 8/8, while the combined two-file package command passed 9/9.
