...
After final production and portability commits `4023bdf`, `e11f2eb`, `cf06c75`, `6e2f3a9`, `fd6b700`, `29d8d07`, `44d9e75`, `633369b`, `89b1b67`, and `b36fe78`, the isolated packed-tarball/generated-CLI Chromium proof `npx playwright test tests/e2e/agent-ready-export.spec.ts --grep "atomically re-exports"` passed **1/1**. Both export requests returned `201`; the second parsed as `exported`, and the stable pair was independently reread after native re-export. The serialized `npm run test:package-export-safety` gate passed **8/8** after its own build; the combined package Vitest run passed **9/9** and its embedded packaged Chromium suite passed **22/22**. The isolated forced-unavailable regression proof `npx playwright test tests/e2e/agent-ready-export-safety.spec.ts --grep "forced unavailable capability"` passed **1/1**, proving refusal before any complete old stable-pair touch.
