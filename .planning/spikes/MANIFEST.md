...
Measure current CLI startup against a representative repository with 10,000 local branches, then cumpa it with staged source discovery that exposes the current branch and registered worktrees before loading searchable branches.
...
| 002 | staged-source-discovery | comparison | Given the same repository, when eager discovery is cumpad with current-branch/worktree-first discovery, then both latency budgets hold without serial per-branch Git calls. | PARTIAL | cli, git, performance, search |
