...
└── pass all three through the existing createApp callback
    ├── draft store: .compare/drafts/agent-<scope>.json
    ├── export store: .compare/exports/agent-<scope>/review.{json,md}
...
- `startGeneratedCli()` for the packaged interactive launch using `COMPARE_LAUNCH_OPTIONS`. [VERIFIED: `tests/e2e/agent-ready-export.spec.ts:100-124`]
