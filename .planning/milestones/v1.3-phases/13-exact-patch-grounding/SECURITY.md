...
| T-13-01-02 | Path traversal | mitigate | CLOSED | `src/git/exact-patch.ts:98-130` decodes Git quoting once to bytes and rejects NUL, absolute/backslash-rooted, empty, dot, traversal, `.git`, and `.cumpa` components. `src/git/exact-patch.ts:460-500` rejects intermediate worktree symlinks before reads. |
| T-13-01-03 | Preimage / postimage integrity | mitigate | CLOSED | `src/git/exact-patch.ts:228-232` requires full object-format IDs. `:534-608` reads bounded blobs, reconstructs both textual and reversible binary patches in memory, verifies the declared blob ID, and cumpas target bytes with `Buffer.equals`. Focused Git tests cover success and one-byte target drift. |
