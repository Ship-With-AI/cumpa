...
- **D-07 (amended 2026-09-11):** Research and document the supported installation/discovery path and invocation form for Claude Code, Codex, Pi and OMP. The user explicitly waived the Phase 6 four-agent installation, discovery, prerequisite and lifecycle test exercise with “ignore this test part” after the isolated authentication setup was explained. Do not request fresh sign-ins or run those tests. Existing helper tests and source/candidate checks remain evidence; four-agent runtime compatibility remains unverified, not passed. Do not invent a blanket "40+ agents verified" claim or omit any target from the published instructions.
...
- **D-11 — execution waiver:** Direct current-chat instruction on 2026-09-11: “ignore this test part”. This supersedes 06-02 Task 3's authentication/proof-assent checkpoint and 06-03 Task 1's target-proof exercise, including its associated released-CLI readiness/abort smoke. It also removes the four-agent public-install test stage from 06-03 Task 3. Record these activities as waived, never as successful observations. No login, installer, provider/model request or review invocation is authorized or needed for this waived exercise. The source skill and exact marketplace candidate remain unchanged. Separate exact marketplace-push approval, candidate-byte checks, push-effects/protection inspection and anonymous post-push public-byte verification remain required. Phase 7 acceptance is unchanged.
- **D-12 — publication authority consumed:** The user supplied “you have approval” for the exact marketplace-only push, then selected “Allow existing automatic actions”, expressly permitting automatic actions already configured for this repository's push, including deployments, without changing integration settings. Main published candidate `984e28c5838176ec15d2af8b996d0307e45b28d5` to `Ship-With-AI/skills` `main` once, then independently verified the public commit and all five file hashes through an anonymous fresh Git fetch. The collection is now `0.3.0`; the marketplace catalog is unchanged. This authority is consumed and does not authorize another push, installer/provider exercise, Cumpa/npm publication, manual Supabase action or integration change. See `06-PUBLICATION-EVIDENCE.json` for the actual assent, attempt and bounded observations.
