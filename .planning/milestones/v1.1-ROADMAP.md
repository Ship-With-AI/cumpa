...
- **v1.1 GitHub Dark Diff** — Phases 05–08 (shipped 2026-07-29)
...
Quick task `260729-gzm` completed the milestone boundary after Phase 08:

- Compacted the changed-files sidebar while preserving selection, counts, accessible labels, and non-color availability cues.
- Made Base/deletion and Head/addition meaning explicit, strengthened whole-line and intraline fills, and retained Monaco line positioning under the production CSP.
- Added `DESIGN.md` and `PRODUCT.md` as the shipped visual-system and product-fact contracts.
- Clarified development-only source-line labels and reserved test fixture contacts without changing production behavior.

The final designer checks reported zero blockers; build, focused Git contracts, and responsive Chromium checks passed.
