...
- **WR-02 — direct-color confinement:** the dependency-free audit now inspects paint-bearing declaration property/value semantics rather than only hex/rgb/hsl text. Outside the one context-free token root, it rejects named colors, every current functional color syntax (`hwb()`, `oklch()`, `oklab()`, `lab()`, `lch()`, `color()`, `color-mix()`, `device-cmyk()`, `light-dark()`, `color-contrast()`, `contrast-color()`), and every standard system color. The terminal forced-colors repair admits only its documented system-keyword allowlist. Semantic variables plus intentionally non-palette `currentColor`/`transparent` values pass deterministic checks; fixtures prove named, modern, and unapproved `AccentColor` values fail.
