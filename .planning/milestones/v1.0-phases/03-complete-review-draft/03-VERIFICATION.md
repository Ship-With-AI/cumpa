...
| DRFT-04 — no unknowingly stale overwrite | VERIFIED | Serialized cumpa-and-swap in `draft-store.ts`; conflict DTO includes expected/actual revisions and latest canonical draft; `App.vue` retains attempts through explicit reload. | API conflict test asserts 409 and no-byte-change; packaged two-tab scenario passed. |
