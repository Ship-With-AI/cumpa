...
| 03-03 | T-03-13 | Repudiation | backup claim | mitigate | CLOSED | `src/server/draft-store.ts:187-204` syncs, rereads, length-checks, and byte-cumpas exclusive backups before returning a safe path; `tests/api/draft-recovery.test.ts:103-124` independently asserts backup Buffer and SHA-256 equality before validating replacement. |
...
| 03-06 | T-03-24 | Spoofing | shortened/display identity | mitigate | CLOSED | `src/git/selector-drift.ts:147-173` cumpas schema-validated full OIDs, while labels are only DTO display fields; `tests/git/selector-drift.test.ts` passed and packaged drift scenarios show complete OIDs. |
