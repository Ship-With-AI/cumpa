...
After final production, portability, and target-aware commits `4023bdf`, `e11f2eb`, `cf06c75`, `6e2f3a9`, `fd6b700`, `29d8d07`, `44d9e75`, `633369b`, `89b1b67`, `b36fe78`, `3679900`, `8179e3b`, and `28b7407`, the canonical serialized `npm run test:package-contract` gate built the runtime and passed **2 files / 12 tests**, including its packaged Chromium suite **22/22**. On declared `darwin-arm64`, the packed target-aware proof `npx playwright test tests/e2e/agent-ready-export.spec.ts --grep "target-aware second export"` passed **1/1**: both export responses were `201`, the second parsed as `exported`, and the stable pair was independently reread after native re-export. The focused native matrix passed **1/1**. The forced-unavailable regression proof `npx playwright test tests/e2e/agent-ready-export-safety.spec.ts --grep "forced unavailable capability"` passed **1/1**, proving typed `reExportUnsupported` before any complete old stable-pair touch; non-darwin-arm64 packed runs now assert the same unsupported response with byte-identical old pair.
