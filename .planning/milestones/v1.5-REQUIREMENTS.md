# Requirements Archive: v1.5 MIT Distribution
...
- **Validated:** 18/19 requirements — all PKG, REL, SKL requirements plus ACC-01, ACC-02, ACC-03.
- **Blocked:** 1/19 — ACC-04. The unpaid and dismissed support states passed on all three
  released installation paths, but every verified-support row is recorded
  `live-entitlement-unavailable` with `substituted: false`, because the paid entitlement
  exists only in Stripe test-mode data and D-07 forbids a new purchase. D-08 mandated this
  honest bounded outcome rather than fabricated, substituted, or reused evidence.
- **Adjusted:** REL-01 was satisfied under the D-10 first-release `latest` exception.
- **Dropped:** none. DIST-01 was already superseded by REL-05 before this milestone.

**Acceptance is specific to `1.5.0`.** The full browser, marketplace, and support-state
acceptance matrix was executed against the public `1.5.0` artifact. The later `1.5.1` and
`1.5.2` patch releases carry their own publication, provenance, and fix-presence evidence,
but they did not inherit that full matrix.

**ACC-04 remains open for the next authorized work:** propagate the false Restore RPC
result into a non-completion response, obtain a permitted genuine live entitlement, then
rerun only the verified-state portions for the global, npx, and marketplace paths.
