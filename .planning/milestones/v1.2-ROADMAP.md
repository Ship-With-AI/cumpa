...
v1.2 makes the ordered source picker useful before Cumpa scans a large local branch namespace, adds literal on-demand branch search through bounded native Git work, and closes only after the production picker path meets its packed-ref latency budgets.
...
**Success Criteria** (what must be TRUE):
  ...
  1. User can interact with the ordered source picker before Cumpa enumerates the remaining local branches.
...
**Success Criteria** (what must be TRUE):
  ...
  1. Launching Cumpa through the production command path against 10,000 packed local branch refs makes the ordered source picker usable within 400 ms of process start.
