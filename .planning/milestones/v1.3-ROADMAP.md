...
Cumpa now accepts a strict, bounded coding-agent request for either a pinned Git range or an exact already-applied patch, reuses its authenticated browser workspace, and returns one canonical JSON result only after explicit Finish.

## Milestone Summary

**Key Decisions:**

- Pin range revisions and ordered native pathspecs once, then carry them through session, draft, and V2 export provenance.
- Ground exact patches from repository preimages and target bytes, serving only private immutable snapshots.
- Keep Finish server-authoritative and one-shot; isolate each attached launch's mutable draft and export storage.

**Issues Resolved:**

- Agent range and exact-patch requests now complete through the existing browser workspace without changing the interactive TTY flow.

**Issues Deferred:**

- No requirement, integration, or end-to-end flow is unsatisfied.

**Technical Debt Incurred:**

- Accepted non-blocking range-diagnostic, UI-contract, patch-status error-handling, Phase 13 security-evidence, and segmented integration-proof debt is recorded in `.planning/MILESTONES.md`.
...
1. A coding agent can pipe one supported, versioned range request into `cumpa` without encountering the interactive source prompt, while a TTY launch still follows the existing ordered source-selection flow.
...
2. Cumpa rejects a patch unless every preimage is repository-grounded and every postimage exactly matches the implemented repository or worktree content.
