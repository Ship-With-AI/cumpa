...
Compare now accepts a strict, bounded coding-agent request for either a pinned Git range or an exact already-applied patch, reuses its authenticated browser workspace, and returns one canonical JSON result only after explicit Finish.
