...
must_haves:
  truths:
    - "Users see Compare everywhere the active product identifies itself, including browser, terminal, exported Markdown, README, product copy, and current project instructions."
    - "The npm package, command, generated executable, environment variables, Monaco IDs, hash domains, export discriminator, and other project-owned machine identifiers use compare/COMPARE/Compare consistently."
    - "New drafts and exports live beneath .compare/, that root is excluded from comparisons, and the optional ignore flow appends exactly /.compare/."
    - "Package metadata, lockfile root metadata, build scripts, package assertions, fixtures, and focused expectations all agree on compare and dist/bin/compare.mjs."
  ...
  artifacts:
    - path: "package.json"
      provides: "compare package name and compare CLI/bin mapping"
      contains: "dist/bin/compare.mjs"
    - path: "package-lock.json"
      provides: "lockfile root metadata synchronized with package.json"
    - path: "src/server/draft-loader.ts"
      provides: ".compare draft path authority"
    - path: "src/server/export-store.ts"
      provides: ".compare export publication and receipt authority"
    - path: "src/git/inventory.ts"
      provides: ".compare internal-path exclusion"
    - path: "src/web/index.html"
      provides: "Compare browser title"
    - path: "README.md"
      provides: "Compare setup, CLI, persistence, export, and shutdown instructions"
  key_links:
    - from: "package.json"
      to: "scripts/build-bin.mjs"
      via: "compare bin mapping targets the generated executable"
      pattern: "compare.*dist/bin/compare\\.mjs"
    - from: "src/contracts/api.ts"
      to: "src/server/export-store.ts"
      via: ".compare receipt-path schemas validate server-produced receipt paths"
      pattern: "\\.compare/exports"
    - from: "src/contracts/draft.ts"
      to: "src/export/review-export.ts"
      via: "compare/export discriminator is shared by schema and producer"
      pattern: "compare/export"
    - from: "src/git/ignore-status.ts"
      to: "src/server/gitignore-capability.ts"
      via: "the .compare probe and /.compare/ append rule describe the same root"
      pattern: "compare.*ignore"
    - from: "src/web/monaco/theme.ts"
      to: "src/web/monaco/diff-adapter.ts"
      via: "Compare theme symbols and compare-owned Monaco identifiers"
      pattern: "COMPARE_THEME|compare-dark"
---

<objective>
Rename the complete tracked active project identity from Diff Review/diff-review to Compare/compare in one clean cutover.

Purpose: Establish Compare as the sole live product, package, CLI, persistence, and machine identity while preserving generic diff terminology and truthful historical records.
...
<constraints>
- Use `Compare` in user-facing product prose and `compare` for package, CLI, generated binary, persistence directory, URI/action/theme IDs, export discriminator, hash domains, temporary fixture prefixes, and other machine identifiers; use `COMPARE_` for project-owned environment variables and constants.
...
<task type="auto">
  ...
  <action>Replace the active project identity end to end: set npm name and sole bin key to `compare`, target `dist/bin/compare.mjs`, and regenerate only lockfile root metadata with `npm install --package-lock-only --ignore-scripts`; update build/verification scripts accordingly. Rename project-owned exported types, schemas, functions, methods, constants, local names, environment variables, Monaco action/model/theme IDs, picker sentinel, native-build fixture prefixes, hash-domain strings, and the canonical export kind to Compare/compare/COMPARE forms, migrating every tracked caller in this task rather than retaining aliases. Change all default draft, export, receipt, inventory-exclusion, ignore-probe, and `.gitignore` rule authorities from `.diff-review` to `.compare`, preserving existing atomicity, symlink, bounded-path, and security behavior. Update terminal/browser/export copy and development prototypes to the `Compare` product name. Keep generic diff-domain identifiers intact. Because the package is private, version 0.0.0, and documented as unpublished, do not add a compatibility CLI, deprecated symbols, or a dual-read persistence path; leave any untracked legacy directory untouched rather than deleting or rewriting user bytes.</action>
  <verify>
    <automated>node -e "const p=require('./package.json'),l=require('./package-lock.json'); if(p.name!=='compare'||p.bin?.compare!=='dist/bin/compare.mjs'||l.name!=='compare'||l.packages[''].name!=='compare'||l.packages[''].bin?.compare!=='dist/bin/compare.mjs') process.exit(1)"</automated>
    <automated>npm run build && test -x dist/bin/compare.mjs && test ! -e dist/bin/diff-review.mjs</automated>
  ...
  <done>The built project exposes only package/CLI `compare`, produces `dist/bin/compare.mjs`, uses `.compare` for all new persistence and ignore behavior, emits Compare product copy, and contains no old project identifiers in active runtime/build source.</done>
</task>

<task type="auto">
  <name>Task 2: Migrate fixtures and assertions to the Compare contract</name>
  ...
  <action>Update every affected fixture, import, symbol reference, environment variable, temporary-directory prefix, package/bin path, persistence path, exported discriminator, Git identity label, browser/terminal assertion, and package evidence record to the Task 1 Compare contract. Preserve the intent and strength of each existing assertion, including near-match inventory cases: translate `.diff-reviewish.txt`, `diff-review.txt`, and nested internal-path fixtures to equivalent `.compare`/`compareish`/`compare.txt` cases rather than deleting them. Do not rename generic diff test concepts. Do not execute Vitest, Playwright, or any project-wide test command in this quick task; the build/package/search gates are the requested verification boundary.</action>
  <verify>
  ...
  <done>All tracked fixtures and assertions describe Compare, `.compare`, `compare/export`, `COMPARE_*`, and `dist/bin/compare.mjs` without weakening persistence, packaging, security, UI-copy, or near-match contracts.</done>
</task>
...
<task type="auto">
  ...
  <action>Rename current user-facing and project-authority prose to `Compare`, including README setup/launch/stop commands, private package identity, generated binary, draft/export locations, ignore-flow copy, PRODUCT facts, the current project heading/description/status/persistence contract, roadmap title, and the loaded `.claude/CLAUDE.md` project/persistence block. Preserve the rationale that Compare reads as English “compare” and Italian “comrade/friend” in the current product description. In `.planning/PROJECT.md`, leave completed phase names and explicitly time-bound v1.0/v1.1 statements unchanged where the old name records what shipped or what a completed milestone was called. Do not edit `.planning/STATE.md`, `.planning/MILESTONES.md`, milestone archives, phase artifacts, debug/forensics records, prior quick plans/summaries, or the checkout directory.</action>
  <verify>
    ...
    <automated>git grep -nE '^# Compare$|^Compare is |^Compare now delivers' -- .planning/PROJECT.md && git grep -nF '.compare/' -- .planning/PROJECT.md && git grep -nE '^# Roadmap: Compare$' -- .planning/ROADMAP.md</automated>
    <automated>git diff --check -- README.md PRODUCT.md .planning/PROJECT.md .planning/ROADMAP.md .claude/CLAUDE.md</automated>
  </verify>
  <done>Live documentation and current planning/instruction identity consistently use Compare while completed historical records remain accurate and the physical checkout root is unchanged.</done>
</task>
...
|---|---|---|---|---|
| GOAL | QUICK-260729-LGA | Complete tracked active rename to Compare/compare | 1, 2, 3 | COVERED |
| CONTEXT | Product contract | Compare in user prose; compare/COMPARE for machine identity | 1, 2, 3 | COVERED |
...
| CHANGE | Persistence | `.compare` drafts/exports, schemas, exclusion, probe, optional ignore append | 1, 2 | COVERED |
...
|---|---|---|---|---|
| T-QUICK-260729-LGA-01 | Tampering | `.compare` persistence boundary | mitigate | Rename path authorities, schemas, exclusion, probe, append rule, and adversarial fixtures together; preserve existing atomic/symlink checks. |
| T-QUICK-260729-LGA-02 | Spoofing | package/CLI identity | mitigate | Require exact package, lockfile, bin, build output, and dry build/search agreement on `compare`. |
...
2. Assert package and lockfile expose only `compare -> dist/bin/compare.mjs`, and the build leaves no old generated executable.
...
<success_criteria>
- Active tracked product prose consistently says Compare.
- Package, CLI command, generated executable, lockfile, build scripts, and package evidence consistently use compare.
- Project-owned machine identifiers use compare/COMPARE naming without aliases or deprecated exports.
- Drafts, exports, receipts, ignore handling, and reviewed-inventory exclusion consistently use `.compare/`.
