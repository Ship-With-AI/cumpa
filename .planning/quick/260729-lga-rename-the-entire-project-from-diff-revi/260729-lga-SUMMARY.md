...
tags: [compare, npm, cli, persistence, vue, typescript]
requires: []
provides:
  - "Clean Diff Review to Compare product, package, CLI, persistence, and machine-identifier cutover"
  - "Compare fixtures, assertions, current documentation, and planning identity"
...
key-files:
  ...
  modified:
    ...
    - .planning/ROADMAP.md
key-decisions:
  - "Compare is the sole active product name; compare and COMPARE own all machine identifiers."
  - "Historical milestone and v1.0 records retain their original Diff Review wording."
patterns-established:
  - "Repository-local persistence, receipt schemas, inventory exclusion, and ignore flow share one .compare root."
...
# Quick 260729-lga: Compare Identity Cutover Summary

**Compare now owns the package, CLI, executable, persistence root, export discriminator, product copy, fixtures, and current project authority without legacy aliases.**
...
- Replaced the npm package/bin identity with `compare` and generated executable with `dist/bin/compare.mjs`.
- Migrated persistence, receipts, ignore handling, inventory exclusion, environment variables, export kind, hash domains, Monaco IDs, callers, and fixtures to Compare.
...
2. **Task 2: Migrate fixtures and assertions to the Compare contract** — `7e94924` (`test`)
...
- PASS — package/lockfile assertion confirmed `compare -> dist/bin/compare.mjs` in both root metadata records.
- PASS — `npm run build && test -x dist/bin/compare.mjs && test ! -e dist/bin/diff-review.mjs` completed successfully after the final runtime-label correction.
- PASS — `npm pack --dry-run --json` produced package `compare@0.0.0` and listed executable `dist/bin/compare.mjs` with mode `493`.
...
- PASS — required project identity checks confirmed `.compare/` in current project authority and `# Roadmap: Compare`.
...
- Kept `Diff Review` in completed milestone and explicit v1.0 historical records in `.planning/PROJECT.md`; current persistence statements changed to `.compare/`.
...
- Compare is ready for normal package, CLI, persistence, and browser workflows under its final identity.
