...
tags: [compare, cumpa, .compare, vitest, playwright]
requires:
  - phase: v1.2
    provides: Compare CLI, .compare persistence authority, and packaged contract suite
provides:
  - sole `spike-findings-compare` skill with current indexes and theme ledger
...
key-files:
  ...
  modified:
    - .claude/CLAUDE.md
    - .kimi-code/skills/spike-findings-compare/
...
key-decisions:
  ...
  - "Corrected stale packaged E2E assertions only after the package contract demonstrated they targeted the removed `dist/bin/compare.mjs` path and obsolete rendered labels."
...
# Quick 260731-hdw: Compare Directory Cutover Summary

**Renamed the sole live Compare spike skill, synchronized live product pointers and `compare-dark`, and proved `.compare` draft/export/ignore/inventory behavior through focused and packaged contracts.**
...
- Moved `.kimi-code/skills/spike-findings-diff-review/` to the sole `.kimi-code/skills/spike-findings-compare/` directory; updated its metadata and bundled product wording.
- Pointed `.claude/CLAUDE.md` and the active spike wrap-up at `spike-findings-compare`; synchronized the current milestone ledger to runtime `compare-dark`.
- Preserved production `.compare` authorities and proved the draft, export, exact inventory exclusion, effective-ignore, package, and theme contracts.
...
2. **Task 2: Prove the `.compare` CLI boundary and absence of intended legacy references** — `b6fef23` (`test`)

**Plan metadata:** Not committed by instruction; this summary remains uncommitted for the orchestrator.

## Files Created/Modified

- `.claude/CLAUDE.md` — live skill display name and invocation now resolve to `spike-findings-compare`.
- `.kimi-code/skills/spike-findings-compare/` — sole renamed skill tree; metadata and bundled staged-discovery wording name Compare.
- `.planning/MILESTONES.md` — current theme ledger uses `compare-dark`.
...
| Command | Result |
| --- | --- |
| `test -f .kimi-code/skills/spike-findings-compare/SKILL.md && test ! -e .kimi-code/skills/spike-findings-diff-review && git grep -nF 'spike-findings-compare' -- .claude/CLAUDE.md .planning/spikes/WRAP-UP-SUMMARY.md .kimi-code/skills/spike-findings-compare/SKILL.md && git grep -nF 'compare-dark' -- .planning/MILESTONES.md src/web/monaco/theme.ts` | Passed: one live skill exists; both live pointers resolve; ledger and runtime source both contain `compare-dark`. |
...
| `npm run test:package-contract` | Initial run exposed stale E2E launch paths (`dist/bin/compare.mjs`) and two stale rendered-text assertions. After their targeted correction, passed: 23 Chromium E2E tests and 12 Vitest package-contract tests. The emitted package artifact was `dist/bin/cumpa.mjs`. |
| `node --input-type=module -e 'import {readFileSync} from "node:fs"; const p=JSON.parse(readFileSync("package.json","utf8")); const l=JSON.parse(readFileSync("package-lock.json","utf8")); if(p.name!=="compare"||p.bin?.cumpa!=="dist/bin/cumpa.mjs"||Object.keys(p.bin).length!==1||l.name!=="compare"||l.packages[""]?.bin?.cumpa!=="dist/bin/cumpa.mjs") process.exit(1)'` | Passed: `compare` package/lock identity and sole `cumpa -> dist/bin/cumpa.mjs` mapping hold. |
...
- Preserved existing production code because the focused contract suite confirmed `.compare/drafts`, `.compare/exports`, exact-root inventory filtering, `.compare` effective-ignore probing, and the append-only `/.compare/` rule already satisfy the required authority boundary.
- Kept the protected Compare, `cumpa`, `dist/bin/cumpa.mjs`, `compare/export`, and `compare-dark` identities unchanged.
...
- **Found during:** Task 2 (Prove the `.compare` CLI boundary and absence of intended legacy references)
- **Issue:** The required `npm run test:package-contract` command failed because two scenarios launched `dist/bin/compare.mjs`; after that repair, assertions expected obsolete composer separators and uppercase context labels.
...
- Live skill/index/ledger identity and `.compare` production boundary are verified.
- No blockers.
