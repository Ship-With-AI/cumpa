...
100%-off comps are fulfillable: a promotion code reaching 100%-off produces a Checkout Session with `payment_status: no_payment_required`, `amount_total: 0`, and no Stripe PaymentIntent, because Stripe charges nothing. The webhook's fulfillment schema accepts that shape as a *comped* session alongside the charged shape, and `support_private.supporters.stripe_payment_intent_id` is nullable so the comped supporter is recordable. Every other anti-forgery fact still applies unchanged: payment mode, USD, `amount_subtotal = 4999`, the exact configured Price, quantity 1, server-owned metadata, and a real Stripe Customer (guaranteed by `customer_creation: "always"`). A comped supporter is verified exactly like a paying one, and may later pay without raising a payment conflict; two distinct charged PaymentIntents for one supporter remain an auditable conflict. Before migration `20260916000000_comped_support_fulfillment.sql` such a session was permanently unfulfillable — the purchaser appeared as a Stripe Customer while the launch prompt kept reappearing.
