...
This is the maintainer evidence policy for `@shipwithai/cumpa@1.5.0`, not a release receipt. The selected source repository is `Ship-With-AI/cumpa`; its Issues tracker is the sole selected problem/question channel. Public readability does not make the application open source.

## Phase 3 — legal and public-source gates

The v1.5 reconciliation (D-20) is complete: the active contract is proprietary distribution with the existing repository and reviewed history becoming public only after authorization. Do not redo the reconciliation or restore the obsolete permanent-private-source premise.

Before consuming the license, verify the exact LICENSE SHA-256 against both named licensors' attributable entries in `.planning/phases/03-distribution-contract-legal-boundary/03-LICENSE-APPROVAL.md`, together with the bound rights review and notices. Missing, unsupported, stale or mismatched evidence blocks continuation. Preserve the difference between actual human approval and an agent-authored claim; a license change requires fresh assent from both licensors. Independently licensed material and prior grants survive.
