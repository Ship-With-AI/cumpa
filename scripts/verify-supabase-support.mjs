...
async function verifyWorkflow(path, options) {
  ...
  const required = [
    ...
    'npm ci', 'npm run build', 'npx playwright install --with-deps chromium', 'npx vitest run --no-file-parallelism', 'npx playwright test --config=tests', 'deno test --allow-env --config supabase/functions/deno.json supabase/functions/tests',
