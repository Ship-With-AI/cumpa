...
const groups = computed(() => projectCommentGroups(props.comments.map((comment) => ({
  id: comment.id,
  state: comment.state,
  body: comment.body,
  side: comment.side,
  ...
  <section class="review-panel" aria-labelledby="review-heading">
    ...
    <section aria-labelledby="open-comments-heading">
      ...
      <template v-for="group in groups.open" :key="group.path.bytesBase64url">
        ...
        <article v-for="comment in group.comments" :key="comment.id" :data-comment-id="comment.id" class="review-panel__comment">
