...
enabled('PostgreSQL recovery schema', () => {
  ...
  test('row lock supports exactly one recovery consumption', async () => {
    ...
    await expect(pool.query("INSERT INTO recovery_challenges (challenge_id, poll_token_hash, magic_token_hash, installation_id, entitlement_id, expires_at, state) VALUES ($1, $2, $3, $4, 1, now() + interval '15 minutes', 'pending')", ['challenge', Buffer.alloc(32, 2), Buffer.alloc(32, 3), 'a'.repeat(43)])).resolves.toBeDefined();
